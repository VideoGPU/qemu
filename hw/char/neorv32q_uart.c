 ...
 * Create UART device.
 */
Neorv32UARTState *neorv32_uart_create(MemoryRegion *address_space, hwaddr base,Chardev *chr)
{
    DeviceState *dev;
    SysBusDevice *s;
    bool succed= false;
...
//    sysbus_connect_irq(s, 0, irq);
    ...
    if (succed) {
		memory_region_add_subregion(address_space, base, sysbus_mmio_get_region(s, 0));
